...
- (id)initWithItem:(id<TDUploadProgressUIDelegate>)item delegate:(id<TDHomeHeaderUploadDelegate>)delegate {
    ...
    if (self) {
        ...
        if ([self.item displayProgressBar]) {
            self.progressBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(offset + 23., 20., 200., 10.)];
            ...
            self.progressBarView = [[UIView alloc] initWithFrame:CGRectMake(offset + 23., 20., 10., 10.)];
            ...
            self.activity.frame = CGRectMake(289., 12., 24., 24.);
        ...
        UIView *bottomBorder = [[UIView alloc] initWithFrame:CGRectMake(0., 49. + (1 / [[UIScreen mainScreen] scale]), 320., 1 / [[UIScreen mainScreen] scale])];
...
- (void)setupLabel {
    if (!self.titleLabel) {
        CGRect labelFrame = CGRectMake(self.thumbnailView ? 63. : 7., 0., self.thumbnailView ? 200. : 280., 49.);
...
- (void)retryUpload {
    ...
    if (self.progressBarView) {
        ...
        CGRect frame = self.progressBarView.frame;
        frame.size.width = 10.0 + (190.0 * [self.item totalProgress]);
        self.progressBarView.frame = frame;
    }
