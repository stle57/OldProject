    ...
    if ([payload objectForKey:@"retryList"]) {
        [[TDAPIClient sharedInstance] sendInvites:[payload objectForKey:@"senderName"] contactList:retryList callback:^(BOOL success, NSArray *contacts) {
             if (success) {
                 ...
                 [[TDAppDelegate appDelegate] showToastWithText:@"Invites failed.  Tap here to retry" type:kToastType_InviteWarning payload:@{@"senderName": senderName, @"retryList": retryList} delegate:[TDAPIClient toastControllerDelegate]];
             }
         }];
    }
}
